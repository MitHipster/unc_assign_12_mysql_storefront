...
SELECT products.prod_id AS ID, products.prod_name AS Product, departments.dept_name AS Department, CONCAT('$', FORMAT(products.price, 2)) AS Price, products.quantity AS Quantity FROM products INNER JOIN departments ON products.dept_id = departments.dept_id ORDER BY departments.dept_id, products.prod_id;
...
SELECT products.prod_id AS ID, products.prod_name AS Product, departments.dept_name AS Department, CONCAT('$', FORMAT(products.price, 2)) AS Price, products.quantity AS Quantity FROM products INNER JOIN departments ON products.dept_id = departments.dept_id WHERE quantity <= 5 ORDER BY quantity ASC, price ASC;
